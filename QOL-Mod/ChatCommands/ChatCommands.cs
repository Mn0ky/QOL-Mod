...
namespace QOL
{
    public class ChatCommands
    {
        ...
        public static void DoubleArgument(string[] cmds, string msg)
        {
            ...
            {
                case "ai":
                    var spawnMethod = AccessTools.Method(typeof(HoardHandler), "SpawnAI");
                    switch (cmds[1])
                    {
                        case "bolt":
                            spawnMethod.Invoke(Helper.Hoards[0], new object[] {Helper.Hoards[0].character});
                            Helper.SendChatMsg("Spawned Bolt.", LogLevel.Success);
                            return;
                        case "zombie":
                            spawnMethod.Invoke(Helper.Hoards[1], new object[] {Helper.Hoards[1].character});
                            Helper.SendChatMsg("Spawned Zombie.", LogLevel.Success);
                            return;
                        default:
                            Helper.SendChatMsg("No AI found.", LogLevel.Warning);
                            return;
                    }
                case "kick":
                    targetID = Helper.GetIDFromColor(cmds[1]);
                    if (Helper.ClientData[targetID].ClientID == MatchmakingHandler.Instance.LobbyOwner)
                    {
                        Helper.SendChatMsg("Cant kick host!");
                        return;
                    }    
                    P2PPackageHandler.Instance.SendP2PPacketToUser(
                        Helper.GetSteamID(targetID), 
                        new byte[] { 0x01 }, 
                        P2PPackageHandler.MsgType.KickPlayer);
                    Helper.SendChatMsg("Kicked: " + Helper.GetColorFromID(targetID));
                    return;
                case "kill":
                    targetID = Helper.GetIDFromColor(cmds[1]);
                    Helper.GetNetworkPlayer(targetID).UnitWasDamaged(1f, true, DamageType.LocalDamage);
                    Helper.SendChatMsg("Killed: " + Helper.GetColorFromID(targetID));
                    return;
                case "sudo":
                ...
                case "gun":
                    ...
                    var weaponWanted = byte.Parse(cmds[1]);
